...
TOKEN = config.BOT_TOKEN  # Token of your BOT
...
async def main():
    ...
    async def start(message: Message, state: FSMContext):
        ...
        catalog = load_catalog()
        await message.answer(
            f"Здравствуйте, {user_name}! Выберите сектор:",
            reply_markup=kb.build_sectors_keyboard(list(catalog.keys())),
        )
    ...
    async def sector_chosen(message: Message, state: FSMContext):
        ...
        if message.text not in catalog:
            await message.answer("❌ Пожалуйста, выберите сектор кнопкой", reply_markup=kb.build_sectors_keyboard(list(catalog.keys())))
            return
